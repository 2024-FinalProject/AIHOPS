...
)  # alias so we don’t shadow your Session factory
...
class DBAccess:
        ...
        with self.lock:  # Ensure thread safety if applicable
            ...
            try:
                ...
                return ResponseSuccessMsg(
                    "Successfully deleted the object from the database."
                )
            except SQLAlchemyError as e:
        ...
        with self.lock:
            ...
            try:
                ...
                return ResponseFailMsg(
                    f"Rolled back, failed to delete from the database"
                )
            finally:
                session.close()  # Close the session
    ...
    def load_by_join_query(
        self, primary_obj, join_obj, select_attrs, join_condition, filter_obj=None
    ):
        session = Session()  # Create a new session
        try:
            # Explicitly specify the primary table using select_from
            query = (
                session.query(*select_attrs)
                .select_from(primary_obj)
                .join(join_obj, join_condition)
            )
    ...
    def delete_all_by_query(self, table, query_obj):
        with self.lock:
            session = Session()
            try:
                session.query(table).filter_by(**query_obj).delete(
                ...
                session.rollback()
                return ResponseFailMsg(
                    f"Failed to delete from {table.__tablename__}: {e}"
                )
            finally:
                session.close()
        ...
        with self.lock:
            ...
            try:
                ...
                with session.begin():
                    ...
                    session.query(DBProjectFactors).filter_by(project_id=pid).delete(
                        synchronize_session=False
                    )
                    session.query(DBPendingRequests).filter_by(project_id=pid).delete(
                        synchronize_session=False
                    )
                    session.query(DBProjectSeverityFactor).filter_by(
                        project_id=pid
                    ).delete(synchronize_session=False)
                    session.query(DBFactorVotes).filter_by(project_id=pid).delete(
                        synchronize_session=False
                    )
                    session.query(DBSeverityVotes).filter_by(project_id=pid).delete(
                        synchronize_session=False
                    )
                    session.query(DBProjectMembers).filter_by(project_id=pid).delete(
                        synchronize_session=False
                    )

                    # finally delete the project row itself
                    session.query(DBProject).filter_by(id=pid).delete(
                        synchronize_session=False
                    )
                # if we reach here, all deletes ran without exception, and session.begin() committed
                return ResponseSuccessMsg(
                    f"Project {pid} and all its dependents deleted."
                )
            except SQLAlchemyError as e:
        ...
        with self.lock:
            ...
            try:
                ...
                with session.begin():
                    ...
                    for proj in owned:
                        pid = proj.id
                        session.query(DBProjectFactors).filter_by(
                            project_id=pid
                        ).delete(synchronize_session=False)
                        session.query(DBPendingRequests).filter_by(
                            project_id=pid
                        ).delete(synchronize_session=False)
                        session.query(DBProjectSeverityFactor).filter_by(
                            project_id=pid
                        ...
                        session.query(DBProjectMembers).filter_by(
                            project_id=pid
                        ).delete(synchronize_session=False)
                        session.query(DBProject).filter_by(id=pid).delete(
                            synchronize_session=False
                    ...
                    session.query(DBMember).filter_by(email=member_email).delete(
                        synchronize_session=False
                    )
                # commit happens automatically on exiting session.begin()
                return ResponseSuccessMsg(
                    f"Member {member_email} and all owned projects deleted."
                )
            except SQLAlchemyError as e:
                session.rollback()
        ...
        with self.lock:
            ...
            try:
                ...
                result = self.update_by_query(
                    DBMember, {"email": email}, {"profile_picture": filename}
                )
