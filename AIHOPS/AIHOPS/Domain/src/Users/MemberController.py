...
class MemberController:
    ...
    def get_users_from_db(self):
        ...
        for member_data in registered_users:
            member = Member(
                member_data.email,
                member_data.encrypted_passwd,
                member_data.id,
                True,
                member_data.verified,
                accepted_tac_version=member_data.accepted_tac_version,
            )
    ...
    def register(self, email, passwd, accepted_tac_version=-1):
        ...
        with self.register_lock:
            ...
            member = Member(
                email, passwd, uid, accepted_tac_version=accepted_tac_version
            )
            # insert to db:
            res = self.db_access.insert(
                DBMember(
                    uid,
                    email,
                    member.encrypted_passwd,
                    accepted_tac_version=accepted_tac_version,
                )
            )
            if not res.success:
                return res
    ...
    def verify(self, email, passwd, code):
        ...
        if not res.success:
            return res
        res = self.db_access.update_by_query(
    ...
    def update_password(self, email, old_passwd, new_passwd):
        with self.register_lock:
            ...
            # update db
            res = self.db_access.update(
                DBMember(member.id, email, updated_member.encrypted_passwd)
            )
            if not res.success:
                return res
    ...
    def start_password_recovery(self, email):
        ...
        return ResponseSuccessMsg(
            f"Started password recovery for {email}, you have 5 minutes to make a new password - please check your email"
        )
    ...
    def recover_password(self, email, passwd, code):
        ...
        res = self.db_access.update_by_query(
            DBMember, {"email": email}, {"encrypted_passwd": member.encrypted_passwd}
        )
        if not res.success:
        ...
        with self.register_lock:
            ...
            if member is not None and member.verified:
                ...
                if not hasattr(member, "is_google_user") or not member.is_google_user:
                    ...
                    res = self.db_access.update_by_query(
                        DBMember, {"email": email}, {"is_google_user": True}
                    )
                    if not res.success:
                        return res
            ...
            return Response(
                True, f"Google user {email} has been registered", member, False
            )
    ...
    def login_with_google(self, email):
        ...
        if not member.verified:
            ...
            member.is_google_user = True
            res = self.db_access.update_by_query(
                DBMember, {"email": email}, {"verified": True, "is_google_user": True}
            )
            if not res.success:
    ...
    def delete_account(self, email):
        ...
        return ResponseSuccessMsg(
            f"Member {email} and all their projects have been deleted."
        )
    ...
    def accept_terms(self, email, version):
        ...
        res = self.db_access.update_by_query(
            DBMember, {"email": email}, {"accepted_tac_version": version}
        )
        if not res.success:
            return res
    ...
    def fetch_profile_picture_from_google(self, token_id, source="google"):
        ...
        try:
            if not hasattr(self, "GOOGLE_CLIENT_ID") or not self.GOOGLE_CLIENT_ID:
                ...
                print(
                    f"Using emergency fallback Google Client ID: {self.GOOGLE_CLIENT_ID}"
                )

            print(
                f"Attempting to fetch Google profile picture with token of length: {len(token_id) if token_id else 0}"
            )
            ...
            if "picture" in id_info:
                ...
                print(
                    f"Initializing Cloudinary with cloud name: {CLOUDINARY_CLOUD_NAME}"
                )
                ...
                print(
                    f"Uploading image from Google URL to Cloudinary with public_id: {email}"
                )
                upload_result = cloudinary_manager.upload_from_url(
                    picture_url, public_id=email
                )
                ...
                if upload_result.success:
                    ...
                    print(
                        f"Successfully uploaded to Cloudinary. URL: {cloudinary_url}, Public ID: {cloudinary_public_id}"
                    )
                    ...
                    if member is not None:
                        print(
                            f"Updating database for member {email} with profile picture: {cloudinary_public_id}"
                        )
                        ...
                        if update_result.success:
                            ...
                            return ResponseFailMsg(
                                f"Failed to update database with Cloudinary info: {update_result.msg}"
                            )
                    else:
                    ...
                    return ResponseFailMsg(
                        f"Failed to upload to Cloudinary: {upload_result.msg}"
                    )
            else:
                print("No profile picture found in the Google token")
                return ResponseFailMsg(
                    "No profile picture found in the user's Google account"
                )
            ...
            traceback.print_exc()
            return ResponseFailMsg(
                f"Failed to fetch profile picture from Gmail: {str(e)}"
            )
    ...
    def update_profile_picture(self, email, filename, source="upload"):
        ...
        try:
            ...
            success = self.db_access.update_by_query(
                DBMember,
                {"email": email},
                {"profile_picture": filename, "profile_picture_source": source},
            ).success
    ...
    def get_terms_and_conditions_version(self, email):
        ...
        return ResponseSuccessObj(
            "Terms and Conditions version", actor.accepted_tac_version
        )
