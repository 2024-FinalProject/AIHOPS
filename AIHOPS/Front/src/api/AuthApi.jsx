...
const logApiCall = (name, ...args) => {
  ...
  return args[args.length-1]; // Return the last argument (usually the promise)
};

export const verify = async (userName, passwd, code) => {
  return logApiCall('verify', {userName, passwd, code: code ? 'REDACTED' : null},
...
export const verifyAutomatic = async (token) => {
  return logApiCall('verifyAutomatic', {token: token ? 'REDACTED' : null},
    await axios.post(`${API_URL}/verify_automatic`, { token })
  );
};

export const register = async (userName, passwd, acceptedTermsVersion = 0) => {
  return logApiCall('register', {userName, passwd: 'REDACTED', acceptedTermsVersion},
...
export const loginUser = async (userName, passwd) => {
  return logApiCall('loginUser', {userName, passwd: 'REDACTED'},
    await axios.post(`${API_URL}/login`, { userName, passwd })
  );
};

export const logoutUser = async () => {
  return logApiCall('logoutUser', {},
...
export const startSession = async () => {
  ...
  logApiCall('startSession', {}, response);
...
export const startPasswordRecovery = async (email) => {
  return logApiCall('startPasswordRecovery', {email},
    await axios.post(`${API_URL}/start_password_recovery`, {
      email,
    })
  );
};
...
export const updatePassword = async (email, password, code) => {
  return logApiCall('updatePassword', {email, password: 'REDACTED', code: code ? 'REDACTED' : null},
...
export const googleLogin = async (tokenId, acceptedTermsVersion = 0) => {
  // Store the Google tokenId for later use (like fetching profile pictures)
  console.log('Storing Google token in localStorage');
  localStorage.setItem('googleToken', tokenId);

  return logApiCall('googleLogin', {tokenId: 'REDACTED', acceptedTermsVersion},
    await axios.post(`${API_URL}/google_login`, {
      tokenId,
      acceptedTermsVersion,
    })
  );
...
export const checkEmailExists = async (tokenId) => {
  return logApiCall('checkEmailExists', {tokenId: 'REDACTED'},
    await axios.post(`${API_URL}/check_email_exists`, { tokenId })
  );
};

export const isValidSession = async (cookie, email) => {
  return logApiCall('isValidSession', {cookie: 'REDACTED', email},
    await axios.get(`${API_URL}/is-valid-session`, {
      params: {
        skipAuth: true,
        cookie: cookie,
        email: email,
...
export const deleteAccount = async () => {
  return logApiCall('deleteAccount', {},
    await axios.post(`${API_URL}/delete_account`, {})
  );
};

export const acceptNewTerms = async (acceptedTermsVersion = 0) => {
  return logApiCall('acceptNewTerms', {acceptedTermsVersion},
    await axios.post(`${API_URL}/accept-terms`, {
      acceptedTermsVersion,
    })
  );
};
