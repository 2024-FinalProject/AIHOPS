...
class VoteManager:
    ...
    def remove_factor(self, fid):
        res = self.db_access.delete_obj_by_query(
            DBProjectFactors, {"project_id": self.pid, "factor_id": fid}
        )
        if res.success:
    ...
    def admin_remove_default_factor(self, fid):
        with self.lock:
            ...
            self.factors_votes = {
                k: v for k, v in self.factors_votes.items() if v
    ...
    def set_factor_vote(self, actor, fid, score, persist=True):
        with self.lock:
            ...
            if cur_vote is not None:
                res = self.db_access.delete_obj_by_query(
                    DBFactorVotes,
                    {"project_id": self.pid, "factor_id": fid, "member_email": actor},
                )
                if not res.success:
        ...
        return ResponseSuccessMsg(
            f"actor {actor}, voted {fid}: {score} on project {self.pid}"
        )

    def set_severity_vote(self, actor, severity_probs):
        with self.lock:
            # Check if a severity vote already exists for this actor and project
            existing_vote = self.db_access.load_by_query(
                DBSeverityVotes, {"actor": actor, "pid": self.pid}
            )
        ...
        return ResponseSuccessMsg(
            f"actor {actor}, voted on severities {severity_probs}"
        )
    ...
    def get_score(self, severity_factors, weights):
        ...
        results = {
            ...
            "severity_damage": {
                "avg": damage_avg,
                "vote_count": self.severity_votes.size(),
            },
            "nominator": N,
            "denominator": D,
            "d_score": d,
    ...
    def get_project_factors_votes(self):
        ...
        return {
            fid: [
                scores[fid] for scores in self.factors_votes.values() if fid in scores
            ]
            for fid in fids
        }
