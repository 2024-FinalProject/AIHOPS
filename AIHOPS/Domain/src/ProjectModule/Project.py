...
class Project:
    def __init__(
        ...
        owner,
        db_access=None,
        is_default_factors=False,
        db_instance=None,
        project_factors=None,
        is_to_research=False,
    ):
    ...
    def set_severity_factors(self, severity_factors):
        ...
        if len(severity_factors) != 5:
            return ResponseFailMsg("only 5 severity factors")
        prev_factor = 0
        for factor in severity_factors:
            if factor < prev_factor:
                return ResponseFailMsg(
                    "severity factors must be in non decreasing order"
                )
            prev_factor = factor
        ...
        return ResponseSuccessMsg(
            f"severity factors have bee nset for project: {self.pid}"
        )
    ...
    def update_name(self, name):
        ...
        self.name = name
        return ResponseSuccessMsg(
            f"name {self.name} has been updated for project: {self.pid}"
        )
    ...
    def update_desc(self, description):
        ...
        return ResponseSuccessMsg(
            f"description {self.desc} has been updated for project: {self.pid}"
        )
    ...
    def get_progress_for_owner(self, pending_amount):
        ...
        return {
            "name": self.name,
            "desc": self.desc,
            "factors_amount": len(self.vote_manager.get_factors()),
            ...
            "member_count": self.members.size(),
        }
    ...
    def remove_member(self, member):
        ...
        try:
            ...
            return ResponseSuccessMsg(
                f"member {member} has been removed from project {self.pid}"
            )
        except ValueError:
            pass
        try:
            ...
            return ResponseSuccessMsg(
                f"member {member} has been from to invite list project {self.pid}"
            )
        except ValueError:
            pass
        return ResponseFailMsg(
            f"member {member} not in to invite nor member of project {self.pid}"
        )
    ...
    def get_factors(self, actor):
        ...
        return ResponseSuccessObj(
            f"project factors for {self.pid}", self.vote_manager.get_factors()
        )

    def get_members(self):
        return ResponseSuccessObj(
            f"project members for {self.pid}", self.members.to_list()
        )

    def get_to_invite(self):
        return ResponseSuccessObj(
            f"project members for {self.pid}", self.to_invite_when_published.to_list()
        )

    def get_severity_factors(self):
        return ResponseSuccessObj(
            f"project severity factors for {self.pid}",
            list(copy.deepcopy(self.severity_factors)),
        )
    ...
    def publish(self):
        ...
        if not self.factors_inited:
            return ResponseFailMsg(
                f"projects {self.pid} factors has not been initialized"
            )
        if not self.severity_factors_inited:
            return ResponseFailMsg(
                f"projects {self.pid} severity factors has not been initialized"
            )
    ...
    def get_score(self, pending_amount, weights):
        try:
            voted_amount = self.vote_manager.get_partially_voted_amount()
            if voted_amount == 0:
                return ResponseFailMsg("voted amount is 0")
            score_res = self.vote_manager.get_score(
                copy.deepcopy(self.severity_factors), weights
            )
    ...
    def get_project_factors_votes(self):
        ...
        return ResponseSuccessObj(
            f"retriveing scores for factors for project id {self.pid}\nscores: {scores}",
            self.vote_manager.get_project_factors_votes(),
        )
    ...
    def to_dict(self):
        return {
            ...
            "factors": [
                ...
                for f in self.vote_manager.get_factors()
            ],
            "factors_inited": self.factors_inited,
            "severity_factors_inited": self.severity_factors_inited,
            ...
            "to_invite": (
                self.to_invite_when_published.to_list()
                if self.to_invite_when_published
                else []
            ),
        }
    ...
    def load_members(self):
        members = self.db_access.load_by_query(
            DBProjectMembers, {"project_id": self.pid}
        )
    ...
    def load_to_invite(self):
        pendings = self.db_access.load_by_query(
            DBPendingRequests, {"project_id": self.pid}
        )
