...
users = [["Alice", "123"], ["Bob", "123"]]

projects = [["Project1", "Desc1"], ["Project2", "Desc2"]]
...
class ProjectTests(unittest.TestCase):
    ...
    def test_update_factor_1_project_1n_design_happy(self, name, use_defaults, apply):
        ...
        self.assertFalse(
            fid in project_factors_ids,
            f"factor should have been deleted from project {fid}",
        )
        #   2) fid not in actors pool
        pool_factors = self.facade.get_actors_factor_pool(users[0][0])
        pool_ids = [d["id"] for d in pool_factors]
        self.assertFalse(
            fid in pool_ids, f"factor should have been deleted from project {fid}"
        )
        #   3) new fid is in actors pool and in project
        self.assertTrue(
            new_fid in project_factors_ids,
            f"new factor should is not added to project {fid}",
        )
        self.assertTrue(
            new_fid in pool_ids, f"new factor should is not added to pactors pool"
        )
    ...
    def test_update_factor_1_project_1n_design_same_name_and_desc_happy(
        self, name, use_defaults, apply
    ):
        self.facade.register_verify_login(*users[0])
        # excpected result shouldnt change with false or tru - add parametrized test here
        pid = self.facade.create_project(users[0][0], use_defaults, *projects[0])
    ...
    def test_update_factor_2_project_1n_design_apply(
        self, name="true", use_defaults=True
    ):
        self.facade.register_verify_login(*users[0])
        pid = self.facade.create_project(users[0][0], use_defaults, *projects[0])
        fid = self.facade.add_factor(users[0][0], pid, *factors1[0])
        ...
        pool_ids = [d["id"] for d in pool_factors]
        self.assertFalse(
            fid in pool_ids, f"factor should not have been deleted from project {fid}"
        )
        #   3) new fid is in actors pool and in project
        self.assertTrue(
            new_fid in project_factors_ids,
        ...
        self.assertTrue(
            new_fid in project_factors_ids,
            f"factor should not been deleted from project {fid}",
        )
        self.assertFalse(
            fid in project_factors_ids, f"new factor should not added to project {fid}"
        )
    ...
    def test_update_factor_2_project_1n_design_not_apply(
        self, name="true", use_defaults=True
    ):
        self.facade.register_verify_login(*users[0])
        pid = self.facade.create_project(users[0][0], use_defaults, *projects[0])
