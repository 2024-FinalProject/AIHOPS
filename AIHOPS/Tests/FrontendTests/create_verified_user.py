...
def create_verified_user():
        ...
        if "invalid code" in error_message.lower():
            ...
            for alt_code in alt_codes:
                ...
                if alt_response and alt_response.get("success"):
                    print(
                        f"   Verification successful with code {alt_code}: {alt_response['message']}"
                    )
                    break
            else:
                print(
                    "   ⚠️ All verification attempts failed, but continuing (user might already be verified)"
                )

    # Step 4: Accept terms (if needed)
    print("4. Accepting terms...")
    terms_data = {"cookie": cookie, "acceptedTermsVersion": TAC_VERSION}
    ...
    if terms_response.get("success"):
        ...
        print(
            f"   Terms acceptance result: {terms_response.get('message', 'Unknown error')}"
        )

    # Step 5: Test login to confirm everything works
    print("5. Testing login...")
    login_data = {"cookie": cookie, "userName": EMAIL, "passwd": PASSWORD}
    ...
    if login_response.get("success"):
        ...
        print(
            f"   Terms version: {login_response.get('accepted_tac_version', 'Unknown')}"
        )
